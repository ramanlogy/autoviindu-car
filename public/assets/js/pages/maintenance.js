...
window.renderMaintenance = function () {
  ...
  var SERVICES = [
    {
      icon: '<i data-lucide="wrench"></i>', color: '#1A6B2A', bg: '#EEF7F0',
    ...
    {
      icon: '<i data-lucide="circle"></i>', color: '#1A4DB8', bg: '#EEF3FC',
      title: 'Brake Service',
    ...
    {
      icon: '<i data-lucide="snowflake"></i>️', color: '#0891B2', bg: '#ECFEFF',
    ...
    {
      icon: '<i data-lucide="zap"></i>', color: '#7C3AED', bg: '#F5F3FF',
    ...
    {
      icon: '<i data-lucide="hammer"></i>️', color: '#B8900E', bg: '#FDF6E0',
    ...
    {
      icon: '<i data-lucide="hexagon"></i>', color: '#C8271E', bg: '#FFF0EF',
  ...
  root.innerHTML =
    ...
    '<div style="width:40px;height:40px;background:var(--green);border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:18px"><i data-lucide="wrench"></i></div>' +
    ...
    '</div>' +
    '<a href="tel:+9779828364940" class="btn btn-primary" style="font-size:14px"><i data-lucide="phone"></i> +977-9828364940</a>' +
    '</div>' +
    ...
    SERVICES.map(function (s) {
      return '<div style="background:var(--white);border:1px solid var(--border);border-radius:var(--r16);padding:22px;transition:all var(--ease)" onmouseenter="this.style.transform=\'translateY(-3px)\';this.style.boxShadow=\'var(--shadow-md)\'" onmouseleave="this.style.transform=\'\';this.style.boxShadow=\'\'">' +
        ...
        s.items.map(function (item) { return '<li style="display:flex;align-items:center;gap:7px;font-size:12px;color:var(--ink-2)"><span style="color:' + s.color + ';font-weight:800"><i data-lucide="check"></i></span>' + item + '</li>'; }).join('') +
        '</ul>' +
        ...
        '<button onclick="alert(\'Book: +977-9828364940\')" style="padding:8px 16px;background:' + s.color + ';color:#fff;border:none;border-radius:var(--r8);font-family:var(--font-body);font-size:12.5px;font-weight:700;cursor:pointer">Book Now</button>' +
        '</div>' +
        '</div>';
    }).join('') +
    '</div>' +
    ...
    PACKAGES.map(function (p) {
      return '<div style="background:var(--white);border:2px solid ' + p.color + '33;border-radius:var(--r16);padding:22px;position:relative">' +
        ...
        p.includes.map(function (item) { return '<li style="display:flex;align-items:center;gap:7px;font-size:12.5px;color:var(--ink-2)"><span style="color:' + p.color + ';font-weight:800"><i data-lucide="check"></i></span>' + item + '</li>'; }).join('') +
        '</ul>' +
        '<button onclick="alert(\'Purchase Package: +977-9828364940\')" style="width:100%;padding:11px;background:' + p.color + ';color:#fff;border:none;border-radius:var(--r10);font-family:var(--font-body);font-size:13.5px;font-weight:700;cursor:pointer">Get This Package</button>' +
        '</div>';
    }).join('') +
    '</div>' +
    ...
    [
      ['<i data-lucide="trophy"></i>', 'Certified Technicians', 'ASE & brand-certified for Toyota, Honda, Hyundai, BYD and more'],
      ['<i data-lucide="hexagon"></i>', 'Genuine OEM Parts', 'No grey-market parts — every part comes with a 6-month warranty'],
      ['<i data-lucide="banknote"></i>', 'Transparent Pricing', 'Fixed price estimates before work begins. No hidden charges'],
      ['<i data-lucide="smartphone"></i>', 'Live Status Updates', 'WhatsApp updates at every stage. See your car\'s progress'],
      ['<i data-lucide="car"></i>', 'Free Pick & Drop', 'Free within Kathmandu ring road for Comprehensive packages'],
      ['<i data-lucide="zap"></i>', 'EV/Hybrid Certified', 'High-voltage battery service, software updates, regen brake care'],
    ].map(function (item) {
    ...
    '<a href="tel:+9779828364940" class="btn btn-primary" style="background:#fff;color:var(--green);font-size:15px;padding:14px 28px"><i data-lucide="phone"></i> Call Now</a>' +
