...
window.renderVariantPage = function (carSlug, variantSlug) {
  ...
  root.innerHTML =
    ...
    '<button class="btn btn-primary btn-full" onclick="alert(\'Best price: +977-9828364940\')">Get Best Price</button>' +
    '<button class="btn btn-gold btn-full" onclick="alert(\'Book Test Drive: +977-9828364940\')">Book Test Drive</button>' +
    ...
    (vr.features || []).map(function (f) {
      return '<div style="display:flex;align-items:center;gap:8px;font-size:13px;color:var(--ink-2)">' +
        '<span style="color:var(--green);font-weight:800;flex-shrink:0"><i data-lucide="check"></i></span>' + f + '</div>';
    }).join('') +
    '</div>' +
    '</div>' +
    '<div style="background:var(--white);border:1px solid var(--border);border-radius:var(--r16);padding:20px">' +
