/* AutoViindu CMS — "Curated Homepage Sections": pick the cars shown in Best
   Sellers, the homepage rows, the New Cars category carousels, Upcoming Cars and
   the "Popular Cars" detail-page sidebar. Saves one SiteContent row:
   curated-sections. The public site reads it via /api/site/curated-sections
   (window.CURATED).

   Ported from the legacy dashboard.html panel. It hooks into cms.html's global
   loadPanel('sections') and reuses cms.html's api()/toast(). */
(function () {
  'use strict';
  ...
  /* cms.html exposes api(method, path, body) -> parsed JSON, and
     toast(msg, 'success'|'error'). Wrap them in the Response-like shape the
     ported builders below expect ({ ok, status, json() }). */
  async function req(pathStr, opts) {
    opts = opts || {};
    var method = opts.method || 'GET';
    var body;
    if (opts.body) { try { body = JSON.parse(opts.body); } catch (e) { body = undefined; } }
    var d;
    try { d = await window.api(method, pathStr, body); }
    catch (e) { return { ok: false, status: 0, json: async function () { return {}; } }; }
    var bad = d && typeof d === 'object' && d.error;
    return { ok: !bad, status: bad ? 400 : 200, json: async function () { return d; } };
  }
  function toast(m, t) { return window.toast(m, t === 'err' ? 'error' : 'success'); }
  ...
  async function loadSections() {
    if ($('topbarTitle')) $('topbarTitle').textContent = 'Curated Homepage Sections';
    var panel = $('sections-panel');
    if (panel) panel.innerHTML = '<div class="empty">Loading sections…</div>';
    try {
      var r = await req('/api/admin/site/' + KEY);
    ...
    if (!inventory.length) {
      try {
        var ir = await req('/api/admin/inventory');
        if (ir.ok) {
          var raw = await ir.json();
          inventory = (Array.isArray(raw) ? raw : []).map(function (c) {
            return {
  ...
  function bestsellersPanel() {
    ...
    return '<p class="section-hint">Drives the <strong>Best Sellers</strong> page and the automatic "Popular Cars" sidebar. Order here = rank. Rows without a catalogue link still show, but are not clickable.</p>' +
      ...
      '<button class="btn btn-secondary" onclick="AVSections.addRow(\'bestsellers.rows\',\'bestseller\')">+ Add row</button>';
  }
  ...
  function upcomingPanel() {
    var rows = data.upcoming.rows.map(function (r, i) {
      ...
      return '<div style="border:1px solid var(--border);border-radius:8px;padding:12px;margin-bottom:10px;background:var(--bg)">' +
        ...
        '<button type="button" class="btn btn-secondary" onclick="this.nextElementSibling.click()">Upload</button>' +
    ...
    return '<p class="section-hint">The <strong>Upcoming Cars</strong> page, the homepage "Upcoming Cars" row and the "Upcoming" carousel on the New Cars page all read this list.</p>' +
      ...
      '<div style="margin-top:14px">' + rows + '</div>' +
      '<button class="btn btn-secondary" onclick="AVSections.addRow(\'upcoming.rows\',\'upcoming\')">+ Add upcoming car</button>';
  }
  ...
  window.AVSections = {
    ...
    uploadImg: async function (path, input) {
      ...
      try {
        var up = await req('/api/admin/media/upload', { method: 'POST', body: JSON.stringify({ imageBase64: b64, filename: file.name }) });
    ...
    save: async function () {
      try {
        var r = await req('/api/admin/site/' + KEY, { method: 'POST', body: JSON.stringify(data) });
